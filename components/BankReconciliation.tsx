...
const BankReconciliation: React.FC<{ user: User; dataOwnerId: string }> = ({ user, dataOwnerId }) => {
  ...
  // Month/year only — no category/verified/bank/search filters — so Categories
  // & Rules can show counts for the selected period without inheriting filters
  // meant for the Transaction Mapping tab.
  const periodBT = useMemo(() => {
    const monthIdx = (MONTH_NAMES.indexOf(selectedMonth) + 1).toString().padStart(2, '0');
    const datePrefix = `${selectedYear}-${monthIdx}`;
    return bankTransactions.filter(t => t.date.startsWith(datePrefix));
  ...
  /**
   * A matched transaction leaves real links behind: the purchase/crew entry it
   * was matched to stays flagged isBankVerified forever unless something clears
   * it, and a record generated by "Push to Purchase Records" has no existence
   * independent of this bank line — deleting the line without deleting that
   * record leaves a phantom purchase counted in every P&L total forever.
   */
  const handleDeleteTransaction = async (bt: BankTransaction) => {
    if (!bt.id) return;

    const matchedIds = bt.matchedPurchaseIds && bt.matchedPurchaseIds.length > 0
      ? bt.matchedPurchaseIds
      : (bt.matchedPurchaseId ? [bt.matchedPurchaseId] : []);

    let warning = 'Delete this bank transaction?';
    if (bt.pushedToPurchases) {
      warning = 'This transaction was pushed to create a purchase record. Deleting it will also delete that purchase record — it has no existence independent of this bank line. Continue?';
    } else if (matchedIds.length > 0) {
      const n = matchedIds.length;
      warning = `This transaction is matched to ${n} bill${n !== 1 ? 's' : ''}. Deleting it will unlink ${n !== 1 ? 'them' : 'it'} — the bill${n !== 1 ? 's' : ''} will go back to unmatched, not disappear. Continue?`;
    }
    ...
    try {
      ...
      if (matchedIds.length > 0) {
        // matchedSource only records the FIRST item's source (see the type
        // comment on BankTransaction) — a combo can mix purchases and
        // crew_entries, so each id is resolved by checking for its actual
        // existence rather than trusted blindly against that one field.
        for (const mid of matchedIds) {
          const tryCollections: ('purchases' | 'crew_entries')[] = bt.matchedSource
            ? [bt.matchedSource, bt.matchedSource === 'purchases' ? 'crew_entries' : 'purchases']
            : ['purchases', 'crew_entries'];
          for (const coll of tryCollections) {
            const snap = await getDoc(doc(db, coll, mid));
            if (!snap.exists()) continue;
            const isBankPushRecord = coll === 'purchases' && (snap.data() as any)._fileId === 'BANK_PUSH';
            if (isBankPushRecord) {
              batch.delete(doc(db, coll, mid));
            } else {
              batch.update(doc(db, coll, mid), { isBankVerified: deleteField() });
            }
            break;
          }
        }
      ...
      batch.delete(doc(db, 'bank_statement_imports', bt.id));
      await batch.commit();
      setBankTransactions(prev => prev.filter(t => t.id !== bt.id));
      await fetchReconciliationData();
    } catch (err) {
      console.error(err);
      alert('Failed to delete the transaction. Check the console for details.');
    }
  };
                      ...
                      <tr key={bt.id} className={`group hover:bg-slate-50/50 transition-colors ${bt.isVerified ? 'opacity-60' : ''} ${selectedBatchIds.has(bt.id!) ? 'bg-indigo-50/30' : ''}`}>
                        ...
                        <td className="px-3 py-4 text-right">
                           <div className="flex items-center justify-end gap-3">
                              ...
                              <button
                                onClick={() => handleDeleteTransaction(bt)}
