...
const PnLHub: React.FC<{ user: User; dataOwnerId: string; readOnly?: boolean }> = ({ user, dataOwnerId, readOnly = false }) => {
  ...
  const [existingPnLSnaps, setExistingPnLSnaps] = useState<PnLMonthlySnapshot[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [sSnaps, eSnaps, emp, rent, adj, pay, setSnap, pnlSnap] = await Promise.all([
        ...
        getDoc(settingsRef),
        getDocs(query(collection(db, 'pnl_snapshots'), ...periodConstraints))
      ]);
  ...
  const pnlData = useMemo(() => {
    ...
    // 1. REVENUE (CASH YIELD)
    const posGoodGross = filteredSalesSnaps.reduce((acc, s) => acc + (s.posGoodGross || 0), 0);
    ...
    const posGoodTax = filteredSalesSnaps.reduce((acc, s) => acc + (s.posGoodTax || 0), 0);
    const posGoodNet = posGoodGross - posGoodTax;
  ...
  const handleFreezeMonth = async () => {
    if (readOnly) return;
    if (!confirm(`Commit and freeze P&L for ${selectedMonth} ${selectedYear}? This will update historical snapshots used for projections.`)) return;
    setIsFreezing(true);
    try {
      const currentFilterOutlets = selectedOutlets.includes('all') 
        ? availableOutlets.map(o => o.id)
      ...
      // We save specific snapshots PER OUTLET to ensure the Forge can look them up individually
      for (const oId of currentFilterOutlets) {
          // Re-calculate for this specific outlet
          const sSnaps = salesSnaps.filter(s => s.outletId === oId);
          const eSnaps = expenseSnaps.filter(e => e.outletId === oId);
          const oRent = rentals.find(r => r.outletId === oId);
          const oPay = monthlyPayrolls.find(p => p.outletId === oId);
          const oAdj = adjustments.find(a => a.outletId === oId);

          const gross = sSnaps.reduce((acc, s) => acc + (s.posGoodGross || 0) + (s.onlineGoodGross || 0) + (s.eventRevenue || 0), 0);
          const inflow = sSnaps.reduce((acc, s) => acc + (s.posGoodNet || 0) + (s.onlineGoodNet || 0) + (s.eventRevenue || 0), 0);
          
          let rawC = 0, rawO = 0, rawL = 0, rawU = 0;
          eSnaps.forEach(snap => {
      ...
      await batch.commit();
      fetchData();
      alert("P&L snapshots successfully frozen for the selected scope.");
    } catch (err) {
      console.error(err);
      alert("Failed to freeze month.");
    } finally {
      setIsFreezing(false);
    }
  };
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight uppercase">P&L Command</h2>
            <p className="text-slate-400 text-sm font-medium uppercase tracking-widest">Monitoring settled revenue vs operational burn.</p>
          </div>
        </div>
             ...
             {!readOnly ? (
               <>
                 <button 
                    ...
                    disabled={isFreezing}
                    className={`px-5 py-2.5 rounded-xl border flex items-center gap-3 transition-all ${pnlData.isMonthFullyLocked ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-500 hover:text-indigo-600 shadow-sm'}`}
                 >
                    {isFreezing ? <Loader2 size={16} className="animate-spin" /> : (pnlData.isMonthFullyLocked ? <ShieldCheck size={16} /> : <Zap size={16} />)}
                    <span className="text-[10px] font-black uppercase tracking-widest">{pnlData.isMonthFullyLocked ? 'Re-Sync Snapshot' : 'Lock P&L Snapshot'}</span>
                 </button>
        ...
        <div className="space-y-12 animate-in fade-in duration-700">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between">
               ...
               <div className="mt-6 flex items-center gap-2 text-slate-300 text-[10px] font-black uppercase"><Store size={12}/> POS Base</div>
            </div>
